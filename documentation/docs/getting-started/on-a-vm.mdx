...
The advanced installer (`install-advanced.sh`) is for production deployments that need a real domain or operator-managed certificates. It is driven by a config file instead of flags and supports three TLS modes. Like the simple installer, it runs **on the VM** with `sudo`.
